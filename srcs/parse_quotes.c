...
void	add_substr_to_cmd(char *input, t_list_command **command, int size, int flags)
{
	char *str;
...
void	simple_quotes(char *input, t_list_command **command, t_parse *par)
{
	if (!par->in_simple && par->i && !ft_in_charset(input[par->i - 1], WHITESPACES))
		...
		add_substr_to_cmd(&input[par->pos], command, par->i - par->pos - 1, NO_SPACE_AFTER);
	else if (par->in_simple)
	{
		if (input[par->i + 1] && !ft_in_charset(input[par->i + 1], WHITESPACES))
			// end quotes without space after
			add_substr_to_cmd(&input[par->pos], command, par->i - par->pos, SIMPLE_QUOTES + NO_SPACE_AFTER);
		else
			// end quotes with space after
			add_substr_to_cmd(&input[par->pos], command, par->i - par->pos, SIMPLE_QUOTES);
	}
	par->pos = par->i + 1;
...
void	double_quotes(char *input, t_list_command **command, t_parse *par)
{
	...
	{
		if (input[par->i + 1] && !ft_in_charset(input[par->i + 1], WHITESPACES))
			add_substr_to_cmd(&input[par->pos], command, par->i - par->pos, DOUBLE_QUOTES + NO_SPACE_AFTER);
		else
			add_substr_to_cmd(&input[par->pos], command, par->i - par->pos, DOUBLE_QUOTES);
	}
	par->pos = par->i + 1;
